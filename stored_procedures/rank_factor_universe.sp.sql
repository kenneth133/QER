...
@AGAINST varchar(1) = NULL,	--optional, defaults to U; values: U,R,Y,C,G
@AGAINST_CD varchar(8) = NULL,--optional if @AGAINST in 'U,R,C,G'; required if @AGAINST = 'Y'; values are country_cd's
@AGAINST_ID int = NULL,		--optional if @AGAINST in 'U,Y' otherise required; values are sector_id, segment_id, region_id
...
IF @FACTOR_SOURCE_CD IS NOT NULL AND NOT EXISTS (SELECT 1 FROM decode WHERE item='SOURCE_CD' AND code=@FACTOR_SOURCE_CD)
  BEGIN SELECT 'ERROR: @FACTOR_SOURCE_CD ' + @FACTOR_SOURCE_CD + ' NOT FOUND IN decode TABLE' RETURN -1 END

IF @GROUPS IS NULL BEGIN SELECT @GROUPS = 100 END
IF @AGAINST IS NULL BEGIN SELECT @AGAINST = 'U' END

IF @AGAINST IS NOT NULL AND @AGAINST NOT IN ('U','R','Y','C','G')
  BEGIN SELECT 'ERROR: @AGAINST MUST BE ONE OF THE FOLLOWING: U, R, Y, C, G' RETURN -1 END
IF @AGAINST IN ('R','C','G') AND @AGAINST_ID IS NULL
  BEGIN SELECT 'ERROR: @AGAINST_ID IS A REQUIRED PARAMETER WHEN RANKING AGAINST SECTOR, SEGMENT, REGION' RETURN -1 END
...
IF @RANK_WGT_ID IS NOT NULL AND NOT EXISTS (SELECT 1 FROM rank_weight WHERE rank_wgt_id = @RANK_WGT_ID)
...
BEGIN
  ...
  BEGIN SELECT 'ERROR: "' + @METHOD + '" IS AN INVALID VALUE FOR @METHOD PARAMETER' RETURN -1 END
END
...
BEGIN
  ...
  BEGIN SELECT 'ERROR: "' + @MISSING_METHOD + '" IS AN INVALID VALUE FOR @MISSING_METHOD PARAMETER' RETURN -1 END
END
IF @MISSING_METHOD IS NULL BEGIN SELECT @MISSING_METHOD = 'MEDIAN' END

INSERT rank_inputs
      (run_tm, as_of_date, bdate, universe_dt, universe_id, factor_id, factor_source_cd, groups,
       against, against_cd, against_id, rank_wgt_id, period_type, method, missing_method, missing_value)
SELECT @RUN_TM, @AS_OF_DATE, @BDATE, @BDATE, @UNIVERSE_ID, @FACTOR_ID, @FACTOR_SOURCE_CD, @GROUPS,
       @AGAINST, @AGAINST_CD, @AGAINST_ID, @RANK_WGT_ID, @PERIOD_TYPE, @METHOD, @MISSING_METHOD, @MISSING_VALUE
...
BEGIN
  ...
  SELECT * FROM rank_inputs WHERE rank_event_id = @RANK_EVENT_ID
END
