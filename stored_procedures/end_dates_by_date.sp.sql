use QER
go
IF OBJECT_ID('dbo.end_dates_by_date') IS NOT NULL
BEGIN
    DROP PROCEDURE dbo.end_dates_by_date
    IF OBJECT_ID('dbo.end_dates_by_date') IS NOT NULL
        PRINT '<<< FAILED DROPPING PROCEDURE dbo.end_dates_by_date >>>'
    ELSE
        PRINT '<<< DROPPED PROCEDURE dbo.end_dates_by_date >>>'
END
go
CREATE PROCEDURE dbo.end_dates_by_date @DATE_TYPE varchar(2) = 'D',
                                       @DATE1 datetime = NULL,
                                       @DATE2 datetime = NULL,
...
IF @DATE_TYPE IN ('YY','YYYY')
  BEGIN
    ...
    SELECT @MAX_DATE = '12/31/' + convert(varchar, datepart(yy, @MAX_DATE))
  END
...
ELSE IF @DATE_TYPE IN ('MM','M')
  BEGIN
    ...
    SELECT @MAX_DATE = dateadd(mm, 1, @MAX_DATE)
  END
ELSE IF @DATE_TYPE IN ('WK','WW')
  BEGIN
    SELECT @DIFF = 1 + abs(datediff(wk, @DATE1, @DATE2))
    SELECT @MAX_DATE = dateadd(dd, 7-datepart(dw, @MAX_DATE), @MAX_DATE)
  END
...
IF @DATE_TYPE IN ('QQ','Q','MM','M')
  BEGIN
    UPDATE #END_DATES_BY_DATE
...
go
IF OBJECT_ID('dbo.end_dates_by_date') IS NOT NULL
    PRINT '<<< CREATED PROCEDURE dbo.end_dates_by_date >>>'
ELSE
    PRINT '<<< FAILED CREATING PROCEDURE dbo.end_dates_by_date >>>'
go