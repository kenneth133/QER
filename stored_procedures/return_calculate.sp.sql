...
IF @RETURN_TYPE = 'BMK' AND @BM_UNIVERSE_ID IS NULL
...
BEGIN
  ...
  BEGIN
    ...
    INSERT return_calc_params (bdate_from, bdate_to, return_type, strategy_id, weight, bm_universe_id, run_tm)
    SELECT @BDATE_FROM, @BDATE_TO, @RETURN_TYPE, @STRATEGY_ID, @WEIGHT, @BM_UNIVERSE_ID, GETDATE()
  END
END
...
IF @RETURN_TYPE = 'ACCT'
BEGIN
  INSERT #POSITION (prev_bdate, univ_type, cusip, sedol, units)
  SELECT bdate, 'TOTAL', cusip, sedol, units
    FROM position
   WHERE bdate >= @BDATE_FROM
     AND bdate <= @BDATE_TO
     AND account_cd = @ACCOUNT_CD
...
BEGIN
  IF @WEIGHT = 'CAP'
  BEGIN
    INSERT #POSITION (prev_bdate, univ_type, cusip, sedol, weight)
    SELECT universe_dt, 'TOTAL', cusip, sedol, weight / 100.0
      FROM universe_makeup
     WHERE universe_id = @BM_UNIVERSE_ID
       AND universe_dt >= @BDATE_FROM
       AND universe_dt <= @BDATE_TO
  END
  ELSE IF @WEIGHT = 'EQUAL'
  BEGIN
    INSERT #POSITION (prev_bdate, univ_type, cusip, sedol)
    SELECT universe_dt, 'TOTAL', cusip, sedol
      FROM universe_makeup
     WHERE universe_id = @BM_UNIVERSE_ID
       AND universe_dt >= @BDATE_FROM
       AND universe_dt <= @BDATE_TO
...
BEGIN
  ...
  BEGIN
    ...
    BEGIN
      ...
      SELECT s.mqa_id, s.ticker, s.cusip, s.sedol, s.isin, s.gv_key,
             s.sector_score, s.segment_score, s.ss_score, s.universe_score, s.total_score
        FROM scores s, instrument_characteristics i
       WHERE s.strategy_id = @STRATEGY_ID
         AND s.bdate = @REBAL_BDATE
         AND i.bdate = @ADATE
         AND s.cusip = i.cusip

      INSERT #SS_SECURITY
      SELECT ss.sector_id, ss.segment_id, ss.mqa_id, ss.ticker, ss.cusip, ss.sedol, ss.isin, ss.gv_key
        FROM #SCORES s, sector_model_security ss
       WHERE ss.bdate = @REBAL_BDATE
         AND ss.sector_model_id = @SECTOR_MODEL_ID
         AND ss.cusip = s.cusip
         AND ss.cusip IS NOT NULL
      ...
      BEGIN
        ...
        SELECT * FROM #SCORES ORDER BY cusip, ticker
        SELECT '#SS_SECURITY'
        SELECT * FROM #SS_SECURITY ORDER BY cusip, ticker
      END
      ...
      INSERT #POSITION (bdate, univ_type, cusip, sedol)
      SELECT @ADATE, 'TOTAL', cusip, sedol
        FROM #SCORES
      ...
      INSERT #POSITION (bdate, univ_type, cusip, sedol)
      SELECT @ADATE, 'UNIVERSE', cusip, sedol
        FROM #SCORES
      ...
      INSERT #POSITION (bdate, univ_type, cusip, sedol)
      SELECT @ADATE, 'SECTOR', cusip, sedol
        FROM #SCORES
      ...
      BEGIN
        ...
        INSERT #POSITION (bdate, univ_type, sector_id, cusip, sedol)
        SELECT @ADATE, 'SECTOR'+CONVERT(varchar,@SECTOR_NUM), d.sector_id, s.cusip, s.sedol
      ...
      INSERT #POSITION (bdate, univ_type, sector_id, cusip, sedol)
      SELECT @ADATE, 'SECTOR'+CONVERT(varchar,@SECTOR_NUM), -1, s.cusip, s.sedol
        FROM #SCORES s, sector_model_security ss
       WHERE s.sector_score <= @UPPER_BOUND
         AND s.sector_score >= @LOWER_BOUND
         AND ss.bdate = @REBAL_BDATE
...
BEGIN
  SELECT '#POSITION (1)'
  SELECT * FROM #POSITION ORDER BY prev_bdate, bdate, univ_type, cusip, sedol
END
...
BEGIN
  ...
  SELECT DISTINCT p.prev_bdate, @DUMMY_UNIVERSE_ID, i.mqa_id, i.ticker, i.cusip, i.sedol, i.isin, i.gv_key
    FROM #POSITION p, instrument_characteristics i
   WHERE NOT EXISTS (SELECT * FROM sector_model_security ss
                      WHERE ss.bdate = p.prev_bdate
                        AND ss.sector_model_id = @SECTOR_MODEL_ID
                        AND ss.cusip = p.cusip)
     AND p.prev_bdate = i.bdate
     AND p.cusip = i.cusip
  ...
  BEGIN
    ...
    SELECT * FROM universe_makeup
     ...
     ORDER BY universe_dt, cusip, sedol
  END
  ...
  BEGIN
    ...
    EXEC sector_model_security_populate @ADATE, @SECTOR_MODEL_ID, @ADATE, @DUMMY_UNIVERSE_ID
  END
...
BEGIN
  ...
  BEGIN
    ...
    INSERT #POSITION (prev_bdate, univ_type, sector_id, cusip, sedol, units)
    SELECT p.prev_bdate, 'SECTOR'+CONVERT(varchar,@SECTOR_NUM), d.sector_id, p.cusip, p.sedol, p.units
      FROM #POSITION p, sector_model_security ss, sector_def d
     WHERE p.univ_type = 'TOTAL'
       AND p.prev_bdate = ss.bdate
       AND p.cusip = ss.cusip
       AND ss.sector_model_id = @SECTOR_MODEL_ID
       AND ss.sector_id = d.sector_id
       AND d.sector_num = @SECTOR_NUM
  END
  ...
  INSERT #POSITION (prev_bdate, univ_type, sector_id, cusip, sedol, units)
  SELECT p.prev_bdate, 'SECTOR'+CONVERT(varchar,@SECTOR_NUM), -1, p.cusip, p.sedol, p.units
...
BEGIN
  ...
  BEGIN
    ...
    INSERT #POSITION (prev_bdate, univ_type, sector_id, cusip, sedol, weight)
    SELECT p.prev_bdate, 'SECTOR'+CONVERT(varchar,@SECTOR_NUM), d.sector_id, p.cusip, p.sedol, p.weight
      FROM #POSITION p, sector_model_security ss, sector_def d
     WHERE p.univ_type = 'TOTAL'
       AND p.prev_bdate = ss.bdate
       AND p.cusip = ss.cusip
  ...
  INSERT #POSITION (prev_bdate, univ_type, sector_id, cusip, sedol, weight)
  SELECT p.prev_bdate, 'SECTOR'+CONVERT(varchar,@SECTOR_NUM), -1, p.cusip, p.sedol, p.weight
    FROM #POSITION p, sector_model_security ss
   WHERE p.univ_type = 'TOTAL'
     AND p.prev_bdate = ss.bdate
     AND p.cusip = ss.cusip
...
BEGIN
  UPDATE #POSITION
   ...
   WHERE #POSITION.prev_bdate = i.bdate
     AND #POSITION.cusip = i.cusip

  UPDATE #POSITION SET price = 1.0 WHERE cusip = '_USD'
  UPDATE #POSITION SET price = 0.0 WHERE price IS NULL
  UPDATE #POSITION SET mval = units * price WHERE units IS NOT NULL
...
INSERT #SECURITY
SELECT i.bdate, i.cusip, i.sedol, i.factor_value
  FROM factor f, instrument_factor i,
      (SELECT DISTINCT bdate, cusip FROM #POSITION) x
...
BEGIN
  SELECT '#SECURITY'
  SELECT * FROM #SECURITY ORDER BY bdate, cusip, sedol
END
